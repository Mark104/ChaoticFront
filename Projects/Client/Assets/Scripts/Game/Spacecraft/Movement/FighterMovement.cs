...
public class FighterMovement : ShipMovement {
	...
	void Update () {
		...
		print ("Adding velocity " + vel + " max speed is " + maxSpeed);
		if(rigidbody.velocity.magnitude < maxSpeed)
		{
			rigidbody.AddForce(vel * Time.deltaTime,ForceMode.Impulse);
		}
		...
		if(currentSendTimer > 0.05f)
		{
			...
			networkView.RPC("SendInput",uLink.RPCMode.Server,Pos,Vel,Rot);
			currentSendTimer = 0;
		}
		else
		{
